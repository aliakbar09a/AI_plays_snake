...
if __name__ == "__main__":
	...
	ap.add_argument('-i', '--input', required=True, help='path of the saved pickle file')
	...
	for saved_snake in snakes:
		...
		while t_snake.isAlive():
			...
			result = t_snake.Brain.decision_from_nn(t_snake.head_x, t_snake.head_y, t_snake.list, t_snake.direction)
			# moving the snake
			...
			if t_snake.steps_taken > 250:
				...
				checkloop = False
			if not alive:
				text = 'Generation : ' + str(generation+1) + '\t\t' + 'Score : ' + str(len(t_snake.list)-1)+'\t[Dead]'
				if t_snake.crash_wall and t_snake.crash_body:
					...
					print('crashed on wall,', 'generation : ',generation+1, 'score : ', len(t_snake.list)-1)
				else:
					print('crashed on body,', 'generation : ',generation+1, 'score : ', len(t_snake.list)-1)
				pygame.display.set_caption(text)
				time.sleep(2)
				break
			if (t_snake.head_x, t_snake.head_y) == arena.food:
				t_snake.steps_taken = 0
				next_position = t_snake.Brain.decision_from_nn(t_snake.head_x, t_snake.head_y, t_snake.list, t_snake.direction)
			...
			screen = t_snake.draw(screen, col.blue)
			pygame.display.update()
			text = 'Generation : ' + str(generation+1) + '\t\t' + 'Score : ' + str(len(t_snake.list)-1)+'\t[Press q to kill]'
			pygame.display.set_caption(text)
			pygame.display.update()
			time.sleep(0.03)
		generation += 1
